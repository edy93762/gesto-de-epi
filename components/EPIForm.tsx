...
export const EPIForm: React.FC<EPIFormProps> = ({ existingEpis, onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ ...formData, id: idUpper, active: true, createdAt: new Date().toISOString() });
  };

  return (
    <div className="max-w-xl mx-auto bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        <section className="bg-slate-950/50 p-6 rounded-3xl border border-slate-800/50 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <input required type="text" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-white text-sm font-black uppercase" value={formData.id} onChange={(e) => setFormData({ ...formData, id: e.target.value })} placeholder="Ex: LUV-01" />
            </div>
            <div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
          ...
          <button type="submit" className="px-10 py-4 bg-white text-black font-black rounded-xl hover:bg-orange-600 hover:text-white transition-all active:scale-95 uppercase text-[10px] tracking-widest">
            Salvar Equipamento
          </button>
        </div>
      </form>
